...
class ArchitectAgent extends base_agent_1.BaseAgent {
    ...
    async process(input) {
        ...
        logger_1.default.info(`${this.name}: Starting architecture design`, { feature: input.feature.feature });
        try {
            ...
            logger_1.default.info(`${this.name}: Architecture design complete`, { processingTime });
            return {
                agentName: this.name,
                data: {
                    architecture,
            ...
            logger_1.default.error(`${this.name}: Error during design`, { error });
            throw error;
        }
    }
    /**
     ...
     */
    async designArchitecture(feature) {
     ...
     */
    async makeTechnicalDecisions(feature, architecture) {
        return [
            {
                id: 'adr-001',
            ...
            },
            {
                ...
                alternatives: architecture.backend.apiStyle === 'rest' ? ['GraphQL', 'tRPC'] : ['REST', 'tRPC'],
            },
        ];
    }
    /**
     ...
     */
    identifyPatterns(_feature, _architecture) {
        return [
            {
     ...
     */
    generateSystemDiagram(_feature) {
        return `
graph TB
    subgraph "Frontend"
        UI[Web UI]
...
`;
     ...
     */
    generateSequenceDiagram(_feature) {
        return `
sequenceDiagram
...
`;
     ...
     */
    generateDiagrams(architecture, feature) {
        return {
     ...
     */
    generateRecommendations(architecture, feature) {
        const recommendations = [];
        recommendations.push(`Use ${architecture.backend.framework} for the backend implementation`);
        recommendations.push(`Implement ${architecture.backend.apiStyle.toUpperCase()} API with OpenAPI documentation`);
